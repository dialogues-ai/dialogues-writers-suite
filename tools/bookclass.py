...
base_dir = "/Volumes/NewVolume/Emotional-Arcs/"
...
class Book_raw_data(object):
    ...
    def load_all_chapters(self):
        ...
        folder = join("/Volumes/NewVolume/Emotional-Arcs/",self.this_Book.txt_file_path)
...
#        print(len(self.word_lists_by_chapter))
        ...
        if isfile(cache_file) and use_cache:
            ...
            if randomize:
                if (len(self.all_word_list) == 0) or (len(self.chunks_nlp) == 0):
                    if isdir(join("/Volumes/NewVolume/Emotional-Arcs/",self.this_Book.txt_file_path)):
    ...
    def __init__(self,dbref):
        ...
        self.cache_dir = "/Volumes/NewVolume/Emotional-Arcs/database/cache/book-data-cache"
        self.chunks_nlp = []
...
def save_book_raw_data(book_raw_data_obj):
    ...
    if use_compression:
        f = open(join("/Volumes/NewVolume/Emotional-Arcs/database/cache",str(book_raw_data_obj.this_Book.pk)+".p.lz4"),"wb")
        f.write(lz4.compress(pickle.dumps(book_raw_data_obj,pickle.HIGHEST_PROTOCOL)))
        f.close()
    else:
        f = open(join("/Volumes/NewVolume/Emotional-Arcs/database/cache",str(book_raw_data_obj.this_Book.pk)+".p"),"wb")
...
def cache_check(b):
    return False
    if use_compression:
        return isfile(join("/Volumes/NewVolume/Emotional-Arcs/database/cache",str(b.pk)+".p.lz4"))
    else:
        return isfile(join("/Volumes/NewVolume/Emotional-Arcs/database/cache",str(b.pk)+".p"))

def load_book_raw_data(b):
    if use_compression:
        f = open(join("/Volumes/NewVolume/Emotional-Arcs/database/cache",str(b.pk)+".p.lz4"),"rb")
        return pickle.loads(lz4.decompress(f.read()))
    # f didn't get closed...
    else:
        return pickle.load(open(join("/Volumes/NewVolume/Emotional-Arcs/database/cache",str(b.pk)+".p"),"rb"))
...
def get_data(q,version,filters,use_cache=True):
    if isfile("/Volumes/NewVolume/Emotional-Arcs/database/cache/timeseries-matrix-cache-{}.p".format(version)) and use_cache:
        big_matrix = pickle.load(open("/Volumes/NewVolume/Emotional-Arcs/database/cache/timeseries-matrix-cache-{}.p".format(version),"rb"))
    ...
    if use_cache:
        pickle.dump(big_matrix,open("/Volumes/NewVolume/books/Emotional-Arcs/database/cache/timeseries-matrix-cache-{}.p".format(version),"wb"),pickle.HIGHEST_PROTOCOL)
    return big_matrix
