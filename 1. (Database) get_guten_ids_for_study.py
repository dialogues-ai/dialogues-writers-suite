...
filteredbooks = []
for book in md.keys():
    if md[book]['downloads'] > 20 and \
    md[book]['language'] == ['en'] and \
    "stories" not in md[book]['title'] and \
    "collection" not in md[book]['title'] and \
    "poems" not in md[book]['title'] and \
    "complete" not in md[book]['title'] and \
    "essays" not in md[book]['title'] and \
    "fables" not in md[book]['title'] and \
    "tales" not in md[book]['title'] and \
    "papers" not in md[book]['title'] and \
    "poetry" not in md[book]['title'] and \
    "verses" not in md[book]['title'] and \
    "ballads" not in md[book]['title'] and \
    "sketches" not in md[book]['title'] and \
    "vol." not in md[book]['title'] and \
    "vols." not in md[book]['title'] and \
    "works" not in md[book]['title'] and \
    "volume" not in md[book]['title'] and \
    "other" not in md[book]['title'] and \
    "literature" not in md[book]['title']:
        filteredbooks.append(book) 

dataset = pd.read_csv('emotional-arc_data.csv')
gut_ids = dataset.iloc[:, 5].values

for book in md.gut_ids():
...
#print first 50 titles of filteredbooks
for book in filteredbooks[:50]:
    print(md[book]['title'])

#print bookid of specific title
settitle = []    
for book in md.keys():
    if md[book]['title'] == "The Evil Guest":
        settitle.append(book)   
...
RDFFILES = '/Volumes/NewVolume/Emotional-Arcs/all_rdf_data.tar'  # The catalog downloaded from Gutenberg
